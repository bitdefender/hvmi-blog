...
Let's first pick up an encoding that is not currently used by any instruction - `0F04`. If we try to decode this isntruction, we will see that bddisasm fails:
```
C:\>disasm -b64 -h 0F04
