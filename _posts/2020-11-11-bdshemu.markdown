...
We will talk in this blog post about the Bitdefender Shellcode Emulator, or [bdshemu](https://github.com/bitdefender/bddisasm) for short. bdshemu is a library capable of emulating basic x86 instructions, while observing shellcode-like behavior. Legitimate code, such as JIT code, will look different compared to a traditional shellcode, so this is what bdshemu is trying to determine: whether the emulated code behaves like a shellcode or not.
